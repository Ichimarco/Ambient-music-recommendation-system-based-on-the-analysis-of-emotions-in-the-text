...
PRESET_SCENARIOS = {
    ...
    # EMOCJA: GNIEW (Dr Roylott) - TEKST ZGODNY Z PG1661
...
def main():
    ...
    with st.sidebar:
        ...
        if 'test_queue' not in st.session_state:
            # Tworzymy "worek" z 10 losami: 5x ALGO, 5x RANDOM
            # To gwarantuje, ze przy wiekszej liczbie prob uzytkownik dostanie po rowno.
            queue = ['ALGO'] * 5 + ['RANDOM'] * 5
            random.shuffle(queue)
            st.session_state.test_queue = queue
            print(f"[SYSTEM] Utworzono kolejke testowa (Block Randomization): {st.session_state.test_queue}")
    ...
    if st.session_state.phase == 'INPUT':
        ...
        if st.button("Wylosuj Tekst i Muzyke (START)", type="primary", use_container_width=True):
            ...
            # 2. Wybieranie metody z KOLEJKI (Zamiast rzutu moneta)
            if st.session_state.test_queue:
                method = st.session_state.test_queue.pop(0)
            else:
                # Jakby ktos zrobil >10 testow, dorabiamy losowo
                method = random.choice(['ALGO', 'RANDOM'])
                
            st.session_state.rec_method = method
            print(f"[SYSTEM] Metoda wybrana z kolejki: {method}. Pozostalo w kolejce: {len(st.session_state.test_queue)}")
    ...
    # FAZA 2: OCENA
        ...
        if st.button("Utwor nie dziala? Wylosuj inny zestaw (Tekst + Muzyka)"):
            ...
            # UWAGA: Przy resecie NIE zmieniamy metody (ALGO/RANDOM), zeby nie psuc balansu w kolejce.
            
            current_method = st.session_state.rec_method
            print(f"[SYSTEM] Zachowuje metode: {current_method} (dla zachowania balansu statystycznego)")
